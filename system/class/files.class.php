...
class files extends baseclass{ 
			...
			while (false !== ($entry = readdir($handle))) {
				if ($entry != '.' && $entry != '..'){
					...
					if (!is_dir($path.'/'.$entry)){
						...
						$foldercontent[$j][$i] = $content.'<br /><a href="edit/?path='.urlencode($path.'/'.$entry).'">'.language::readType('EDIT_VIEW').'</a> 
						| <a href="delete/'.$entry.'">'.language::readType('DELETE').'</a>';
					}
	...
	public static function folderAdminLister($path,$showClass = true){
		$foldercontent = array();
		$line = array();
		$width = '32';
		if ($handle = opendir($path)) {
			while (false !== ($entry = readdir($handle))) {
				if ($entry != '.' && $entry != '..' && (($entry != 'class' && strpos($entry,'control') === false && !$showClass) || $showClass)){
					...
					if (is_dir($path.'/'.$entry)){
						$line[] = '&nbsp;';//form::check(0,form::encode($entry));
						$line[] = '<img src="/template/'.TEMPLATE.'/icon_big/folder.png" width="'.$width.'px;" />';	
					} else {
						$line[] = form::check(0,form::encode($entry));
						$line[] = self::getIcon($filetype,$width);				
					}
					$line[] = '<a href="/system/structure/edit/?path='.form::encode($tmp).'">'.$entry.'</a>';
					$line[] = '<a href="/system/structure/download/?path='.form::encode($path).'&file='.$entry.'" class="btn btn-primary btn-xs">Download</a>';
					$line[] = '<a href="javascript:confirmDel(\'/system/structure/edit/delete/?path='.form::encode($path).'&file='.$entry.'\')" class="btn btn-danger btn-xs"> Slet </a>';						
				}
